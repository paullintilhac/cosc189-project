...
def recons_defense(model_dict, input_var, target_var, test_prediction, dev_list,
    ...
    for mag_count in range(len(dev_list)):
        ...
        recons_adv = pca.inverse_transform(X_adv_dr).reshape((test_len, 1,
                                                              height, width))
        output_list.append(acc_calc_all(recons_adv, y_test, X_test_rev, i_c, validator, indexer, predictor, confidence))
        err, acc = validator(recons_adv, y_test)
        print("for epsilon = " + str(dev_list[mag_count]))
...
def retrain_defense(model_dict, data_dict, input_var, target_var,
    ...
    for mag_count in range(len(dev_list)):
        X_adv_dr = pca.transform(adv_x_all[:,:,mag_count]).reshape((test_len,
                                                                    1, rd))
        output_list.append(acc_calc_all(X_adv_dr, y_test, X_test_dr, i_c, validator, indexer, predictor, confidence))
        err, acc = validator(X_adv_dr, y_test)
        print("for epsilon = " + str(dev_list[mag_count]))
        print("real test accurcy: " + str(acc))
    # Printing result to file
    print_output(model_dict, output_list, dev_list, is_defense=True, rd=rd)
